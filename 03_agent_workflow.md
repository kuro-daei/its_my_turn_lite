...
MCP サーバー（基盤）
└── Web 検索・GitHub などの外部サービスへのアクセスを提供
```
...
MCP サーバーを追加することで、Claude Code が Web 検索・GitHub 操作などの外部サービスを直接利用できるようになります。
...
| MCP サーバー | Web 検索・GitHub 連携 | `claude mcp add` |
...
3. **MCP サーバー** — Brave Search・GitHub を `claude mcp add` で接続

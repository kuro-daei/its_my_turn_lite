...
ツールのセットアップを始める前に、以下の資料で Git・GitHub と Pull Request の基本概念を確認しておきましょう。

### Bash / ターミナル

ターミナル（黒い画面）でコマンドを入力して操作する環境のことを **Bash（バッシュ）** と呼びます。普段マウスでクリックしている操作を、文字で指示する感覚です。最初は戸惑いますが、慣れると圧倒的に速く作業できます。

- [動画: Bash・ターミナル入門](https://youtu.be/FrBe7Vv0-K4?si=Ls53kL_vKu3p_Msw)
...
- [動画: Git・GitHub入門](https://youtu.be/LDOR5HfI_sQ?si=7Fk-xOXzeokFn4d2)
- [記事: Git・GitHubとは？](https://www.creativevillage.ne.jp/category/topcreators/web-creator/webprogrammer/128504/)

### Pull Request & Review

Pull Request（プルリクエスト、通称「ぷるりく」）は、「この変更をプロジェクトに取り込んでください」と提案する仕組みです。チームで開発するときの「確認・承認フロー」にあたります。レビューの流れと合わせて使い方を知っておきましょう。

- [動画: Pull Requestの使い方①](https://www.youtube.com/watch?v=zeX2KASkOXY)
- [動画: Pull Requestの使い方②](https://youtu.be/euK7YazfN3w?si=AQolTtOoYHHYa6lh)
- [記事: Pull Requestとは？](https://qiita.com/obscure723/items/5265556d1b89e77c456b)
...
> [!WARNING]
> インストール中に Xcode Command Line Tools のインストールも自動的に行われる場合があります。その場合は、追加で数分かかります。
...
> [!WARNING]
> Homebrew でインストールした場合、この設定は自動で行われている場合もあります。
...
> [!WARNING]
> `which git` を実行したとき、`/opt/homebrew/bin/git`（Apple Silicon）または `/usr/local/bin/git`（Intel）が表示されれば、Homebrew 版の Git が使われています。`/usr/bin/git` と表示された場合は、macOS プリインストール版が使われています。PATH の設定を確認してください。
...
> [!WARNING]
> `user.email` には GitHub に登録したメールアドレスを使ってください。
...
> [!IMPORTANT]
> npm を使った `npm install -g @anthropic-ai/claude-code` でのインストールは
> **2025年以降は非推奨**となっています。
